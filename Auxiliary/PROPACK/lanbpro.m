...
if isnumeric(A) | isstruct(A)
   ...
   if (narg < 5); error('Not enough input arguments.'); end
   Atrans = varargin{2};
   if ~isstr(Atrans)
   ...
   if (narg == 12); Apar = varargin{12}; else; Apar = []; end
end
...
elseif  (min(m,n) == 1)
   if isnumeric(A)
      ...
      U = 1;  
      if ~isempty(Apar); B_k = feval(A,1,Apar); 
      else; B_k = feval(A,1); 
      end
      V = 1; p = 0; ierr = 0; work = zeros(2,2);
   end
   if (nargout<3); U = B_k; end
   return;
end
...
if isempty(U)
   ...
   int = [1:j]';
   if (exist('mexreorth') == 3)
      [p,beta(j+1),rr] = mexreorth(U,p,beta(j+1),int,gamma,cgs);
   else
...
for j = j0:k
   ...
   if (j==1)
      ...
      elseif isstruct(A)
         ...
         if ~isempty(Apar); r = feval(Atrans,U(:,j),Apar); 
         else; r = feval(Atrans,U(:,j)); 
         end
         r = r - beta(j)*V(:,j-1);
      end
      alpha(j) = norm(r); 
      ...
      if (onesided~=-1) & (fro | numax(j) > delta | force_reorth) & (alpha(j)~=0)
	    ...
	    int = compute_int(nu,j-1,delta,eta,0,0,0);
         ...
         %% Reorthogonalize v_j 
         if (exist('mexreorth') == 3)
            [r,alpha(j),rr] = mexreorth(V,r,alpha(j),int,gamma,cgs);
         else
   ...
   if (alpha(j) < max(n,m)*anorm*eps) & (j<k), 
      ...
      for attempt=1:3    
	    ...
	    r = A.R\r;    
            ...
            if ~isempty(Apar); r = feval(Atrans,r,Apar);
            else; r = feval(Atrans,r);
            end
         end
         nrm=sqrt(r'*r); % not necessary to compute the norm accurately here.
         int = [1:j-1]';
         if (exist('mexreorth') == 3) 
            [r,nrmnew,rr] = mexreorth(V,r,nrm,int,gamma,cgs);
         else
...
%%----- modified by KCT ------
  ...
  elseif isstruct(A)
     ...
     if ~isempty(Apar); p = feval(A,V(:,j),Apar); 
     else; p = feval(A,V(:,j)); 
     end
     p = p - alpha(j)*U(:,j);
  end
  ...
  if (beta(j+1) < max(m,n)*anorm*eps)  & (j<k)     
      ...
      for attempt=1:3    
	    ...
	    p = A.Rt\p;
   	    ...
   	    if ~isempty(Apar); p = feval(A,p,Apar);
            else; p = feval(A,p);
            end
         end
         nrm = sqrt(p'*p); 
         int = [1:j]';
         if (exist('mexreorth') == 3)
            [p,nrmnew,rr] = mexreorth(U,p,nrm,int,gamma,cgs);
         else
