function [x, histout, theta, itc, fail] = LRGeomCG_bin(prob, opts, x0, fc0, rel_inner_tol)
...
%   fprintf('Iteration %d\n',itc)
  fc_b = fc;
  [xc_new,fc,succ,~,iarm] = armijo_search(prob, xc, fc, gc, dir, 1.2*tinit, theta);
  ...
  if ~succ && beta ~= 0
    ...
    [xc_new,fc,succ,~,iarm] = armijo_search(prob, xc,fc_b,gc,dir, 1.2*tinit);
  end
